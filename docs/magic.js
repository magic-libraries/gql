...
var view = C('view');
var initialState = {
  'description': 'graphql query handler for @magic',
  'gdpr': {
    'allowed': [],
    'show': true
  },
  'logotext': '@magic-libraries/gql',
  'menu': [{
    'text': 'installation',
    'to': '/#installation'
  }, {
...
var lib = {
  ...
  gql: function () {
    var gql = function gql(query) {
      ...
      var name = /(query|mutation) ?([\w\d-_]+)? ?\(.*?\)? \{/.exec(query);
...
var pages = {
  '/gql/': function gql(state) {
    return [h2(state.title), p([Link({
      to: 'https://magic.github.io/core'
    }, '@magic'), '-client library: ', state.description]), GitBadges('magic-libraries/gql'), h3({
      id: 'installation'
    }, 'installation'), Pre('npm install --save-exact @magic-libraries/gql'), h3({
      id: 'usage'
    }, 'usage'), p('in a page/component, just use the lib.gql functions'), Pre("\nconst query = lib.gql(`\nquery getHuman($id: Int = 3) {\n  human(id: $id) {\n    name\n    height\n  }\n}\n`)\n\nconst jsonString = query({ id: 1 })\n"), p('jsonString now is valid json:'), Pre("\n{\n  'query': `query getHuman($id: Int = 3) {\n    human(id: $id) {\n      name\n      height\n    }\n  }`,\n  'variables': { id: '1' },\n  'operationName':'getHuman'\n}"), h3({
      id: 'source'
